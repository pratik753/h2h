...
form.keepExtensions=true;
  ...
  const newProduct = await pool.query(
    "insert into products (title,descriptio,image_name,price,email) values ($1,$2,$3,$4,$5)",
    [title, descriptio, image_name, q, email]
  );
...
router.post(
  ...
  ID1  SERIAL PRIMARY KEY*/
    ...
    const newProduct = await pool.query(
      "insert into Doctor (reimbersement,dat,email,item,img) values ($1,$2,$3,$4,$5)",
      [reimbersement, dat, email, p, im]
    );
    console.log("yes ", newProduct);
  }
);
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(pool);
      const newProduct = await pool.query(
        "insert into Prescription (title,price,quantity,email) values ($1,$2,$3,$4)",
        [title, price, quantity, email]
      );
...
router.get("/dealer", verifyToken, restrictTo("Dealer"), async (req, res) => {
  try {
    ...
    const data = await pool.query(
      "SELECT * FROM Userss WHERE position = $1 or position=$2",
      [positiondoctor, positionnurse]
    );
    let arr = [];
...
router.get("/getpres", verifyToken, async (req, res) => {
  try {
    const { id } = req.user;
    console.log("hi from prescription", id);
    const data = await pool.query("SELECT * FROM products WHERE email=$1", [
      id,
    ]);
...
router.get("/alldealer", async (req, res) => {
  try {
    ...
    const getall = await pool.query(
      "SELECT * FROM Userss WHERE position = $1",
      [email]
    );
    console.log(getall);
...
router.get(
  "/getall",
  verifyToken,
  restrictTo("Doctor", "Nurse", "Dealer"),
  async (req, res) => {
    try {
      const { id } = req.user;
      const email = id;
      ...
      for (let i = 0; i < data.rows[0].dealer.length; i++) {
        ...
        val = await pool.query("SELECT * FROM products where email=$1", [
          value,
        ]);
...
router.get(
  ...
  restrictTo("Dealer"),
  async (req, res) => {
    //prescription toh doctor and nurse add kr sakte hai..
    try {
      const { email } = req.body;
      const data = await pool.query(
        "SELECT * FROM Prescription WHERE email = $1",
        [email]
      );
      // console.log(data);
...
router.post("/getdate", verifyToken, restrictTo("Dealer"), async (req, res) => {
  //prescription toh doctor and nurse add kr sakte hai..
  try {
    let { id } = req.user;
    let email = id;
    ...
    for (let i = 0; i < data.rows.length; i++) {
      if (data.rows[i].dealer != null && data.rows[i].dealer.includes(email)) {
        ...
        //  console.log(em);
        const dat = await pool.query("SELECT * FROM Doctor where email=$1", [
          em,
        ]);
...
router.get("/orders", verifyToken, async (req, res) => {
  ...
  const email = id;
  try {
    let data = await pool.query("SELECT * FROM doctor WHERE email=$1 ", [
      email,
    ]);
    let n = data.rows.length;
    let adding = [];
    for (let i = 0; i < n; i++) {
      let title = data.rows[i];
      adding[i] = title;
    }
    ...
    if (startdate != " " && enddate != " ") {
      if (company == " ") {
        data = await pool.query(
          "SELECT * FROM Prescription WHERE email = $1 and dat between startdate=$2 and enddate=$3",
    ...
    if (startdate != " " && enddate != " ") {
      if (company != " ") {
        ...
        for (let i = 0; i < n; i++) {
          let title = dealername.rows[i].title;
          data = await pool.query(
            "SELECT * FROM Prescription WHERE title = $1 and dat between startdate=$2 and enddate=$3 ",
