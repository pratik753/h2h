...
function Login() {
  ...
  function cleardata()
  {
   ...
   Object.assign(formData, { password:password});   
    ...
    if(response.data.rows[0].position==='Doctor'||response.data.rows[0].position==='Nurse')
    {
      console.log("bye",response.data.rows[0].position);
  ...
  //  localStorage.setItem("",response.data.accesToken);
    //  req.headers.Authorization = `Bearer ${JSON.parse(localStorage.getItem('profile'))}`;
   }
  return (
    <div class="h-full md:h-screen md:flex py-10 " onLoad={cleardata}>
